...
8. Only exact-head `VALIDATION_PASS` may proceed to independent code review. The canonical review provider is the `Claude PR Review` GitHub Actions workflow using `anthropics/claude-code-action@v1`, dispatched deterministically by `scripts/rick-loop-review-dispatch.mjs` — never by a push. Validation asks whether the implementation proves the spec; review asks whether the implementation is sound. Never merge the two gates.
9. The raw v1.3.x kernel may still emit the compatibility name `WAIT_FOR_CODEX`; in v1.5 interpret it as `WAIT_FOR_INDEPENDENT_REVIEW`. `WAIT_FOR_CODEX`, `WAIT_FOR_CI`, `EXTERNAL_RETRYABLE`, or legacy retryable `BLOCKED_EXTERNAL` are handled by `node scripts/rick-loop-watcher.mjs`. For reviewer waits, the watcher observes/reruns the Claude review workflow; it must not request Codex and must not ask the owner to type `resume`.
...
A clean Claude review is accepted only when its top-level result names the exact HEAD as `Reviewed commit: <sha>`, contains the explicit clean verdict `No major issues found.`, comes from an author independent of the PR author, and there are zero unresolved inline findings for that exact HEAD.

Any HEAD change invalidates the result. A new review is **not** triggered by the push: since ARCH-04, `claude-pr-review.yml` has no `pull_request` trigger at all, and a push never invokes an LLM. `scripts/rick-loop-review-dispatch.mjs` owns dispatch and fires exactly one review per HEAD, only once CI is green on that exact HEAD, the deterministic preflight passes, and no existing run or published verdict already covers it. The watcher calls it; there is no manual `@claude` command.

Removing the automatic execution never relaxed the gate. The review remains mandatory, independent and exact-head before merge, and `evaluateMergeAllowed` is unchanged.
